    ...
    def classifier_fn(sentences):
        ...
        for i in range(0, input_ids.size(0), bs):
            ...
            if hasattr(model, "roberta"):
                output = model.roberta(input_ids=input_ids[i:i0].to(device),
                                       attention_mask=attention_mask[i:i0].to(device))
                output = output[0]
                ...
                output = model.bert(input_ids=input_ids[i:i0].to(device),
                                    attention_mask=attention_mask[i:i0].to(device))
                if pooled_output:
                    cls_reps = output[1]
                else:
...
def grey_color_func_colored(word, font_size, position, orientation, random_state=None,
                            **kwargs):
    sia = SentimentIntensityAnalyzer()
    scores = sia.polarity_scores(word)
    pos = int(((1 + scores["compound"]) / 2) * 255)
    return (pos, 255 - pos, 120)
...
def plot_wordcloud_colored(expln_dict, weights, factor=3, transpose=False,
    ...
    for i, idx in enumerate(expln_dict.keys()):
        ...
        for j in [0, 1]:
            ...
            if transpose:
                ...
                ax = axs[i, j]
            ax.imshow(wc.recolor(color_func=grey_color_func_colored, random_state=3),
                      interpolation="bilinear")
            ax.set_xticks([])
            ax.set_yticks([])
            ax.spines['right'].set_visible(False)
